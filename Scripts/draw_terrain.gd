...
func initialize_render(framebuffer_format : int):
	...
	var vert_code := FileAccess.open("res://Shaders//source_vertex.vert", FileAccess.READ).get_as_text()
	var frag_code := FileAccess.open("res://Shaders//source_fragment.frag", FileAccess.READ).get_as_text()

	p_shader = compile_shader(vert_code, frag_code)
